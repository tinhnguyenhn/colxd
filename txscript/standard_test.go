...
// newAddressPubKey returns a new godashutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) godashutil.Address {
	addr, err := godashutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new godashutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) godashutil.Address {
	addr, err := godashutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new godashutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) godashutil.Address {
	addr, err := godashutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the godashutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the godashutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := godashutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := godashutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := godashutil.NewAddressPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := godashutil.NewAddressPubKey(decodeHex("03b0bd"+
	...
	p2pkUncompressedMain, err := godashutil.NewAddressPubKey(decodeHex("0411db"+
	...
	tests := []struct {
		...
		{(*godashutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*godashutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*godashutil.AddressPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := godashutil.NewAddressPubKey(decodeHex("02192d7"+
	...
	p2pkUncompressedMain, err := godashutil.NewAddressPubKey(decodeHex("0411d"+
	...
	tests := []struct {
		keys      []*godashutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*godashutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
