...
// newAddressPubKey returns a new colxutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) colxutil.Address {
	addr, err := colxutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new colxutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) colxutil.Address {
	addr, err := colxutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new colxutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) colxutil.Address {
	addr, err := colxutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the colxutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the colxutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := colxutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := colxutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := colxutil.NewAddressPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := colxutil.NewAddressPubKey(decodeHex("03b0bd"+
	...
	p2pkUncompressedMain, err := colxutil.NewAddressPubKey(decodeHex("0411db"+
	...
	tests := []struct {
		...
		{(*colxutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*colxutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*colxutil.AddressPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := colxutil.NewAddressPubKey(decodeHex("02192d7"+
	...
	p2pkUncompressedMain, err := colxutil.NewAddressPubKey(decodeHex("0411d"+
	...
	tests := []struct {
		keys      []*colxutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*colxutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
