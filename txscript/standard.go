...
// specified address.
func PayToAddrScript(addr colxutil.Address) ([]byte, error) {
...
func MultiSigScript(pubkeys []*colxutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []colxutil.Address, int, error) {
	var addrs []colxutil.Address
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := colxutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := colxutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := colxutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]colxutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := colxutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
