...
func addrToKey(addr colxutil.Address) ([addrKeySize]byte, error) {
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[wire.ShaHash]*colxutil.Tx
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *colxutil.Block, view *blockchain.UtxoViewpoint) {
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *colxutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *colxutil.Block, view *blockchain.UtxoViewpoint) error {
	// Build all of the address to transaction mappings in a local map.
	addrsToTxns := make(writeIndexData)
	idx.indexBlock(addrsToTxns, block, view)
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr colxutil.Address, numToSkip, numRequested uint32, reverse bool) ([]database.BlockRegion, uint32, error) {
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil, 0, err
	}
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *colxutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[wire.ShaHash]*colxutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *colxutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr colxutil.Address) []*colxutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*colxutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[wire.ShaHash]*colxutil.Tx),
